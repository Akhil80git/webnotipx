...
(async function ensureOwner(){
  ...
  if (!owner) {
    ...
    owner = new User({ email: ownerEmail, passwordHash: hash, name: 'Owner', isOwner: true, notify: false }); // Owner notify default false
    await owner.save();
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const user = new User({ name, email, passwordHash: hash, isOwner: false, notify: false }); // Default notify false for new users
    await user.save();
...
// Owner endpoint to send notification to all users who are notify=true and have a subscription, exclude owner
app.post('/api/sendNotification', authMiddleware, async (req, res) => {
  try {
    ...
    // find users with notify=true, exclude owners
    const users = await User.find({ notify: true, isOwner: false });
